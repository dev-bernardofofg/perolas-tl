...
- **Identidade leve** (`src/server/identity.ts` + `identity.server.ts`): o ator É uma `Person` — escolhida no modal "Quem é você?" (footer, `WhoAmI`), cookie `perolas_who` HttpOnly com o personId (180 dias). Sem senha por design: trocar de identidade é livre pela UI, então assinar o cookie seria teatro — quem protege o histórico é a auditoria. **Escrever exige identidade**: `requireActorId()` em toda server function de escrita (backstop) + gate client-side (`requestIdentity()` abre o modal). REGRA DE OURO do Start: módulos com server functions só podem exportar server functions — helpers puros que usam `react-start/server`/prisma vivem em arquivos `.server.ts` (senão o import-protection barra o bundle client; bug real desta sessão).
- **Autoria e auditoria**: `Utterance.actorId` (quem clicou; null = era pré-identidade) e tabela `audit_logs` (`logAction` em `audit.server.ts`) gravada por TODA escrita — resumo humano desnormalizado sobrevive à exclusão do que descreve; edição guarda antes/depois no payload; auditoria nunca derruba a ação (catch + console). `mergePeople` migra também `actorId` de utterances e logs. `/auditoria` (master-only, `getAuditLog` com `masterMiddleware`; link no header só aparece para master). Feed mostra o ator ("X deu +1 em ... de Y"); ranking ganhou **Incentivadores** (+1s dados, exclui a utterance de registro) e **Deduradores** (registros) — seções escondem quando vazias.
- **Modo master** (`src/server/auth.ts`): curadoria (deletar/editar pérola, mesclar/remover pessoas) exige master; +1/−1 e registrar continuam livres. Sessão **stateless**: token `exp.HMAC-SHA256(exp, MASTER_PASSWORD)` em cookie `perolas_master` HttpOnly/SameSite=Lax (Secure em prod), 7 dias — trocar a senha derruba todas as sessões. **A fronteira é o `masterMiddleware`** anexado às 4 server functions de curadoria; a UI (query `['master']` → `getMasterStatus`) só esconde botões. Login/logout no footer (`MasterPanel`). Env: `MASTER_PASSWORD` (dev em `.env.local`; **configurar na Vercel** — sem ela o login responde "não configurado"). Ler cookie/env só dentro de handler/middleware, nunca em module scope.
- **Busca da home**: filtro client-side sobre a coleção já sincronizada (`searchable()` em `normalize.ts` — acento-insensível), match em texto+contexto+autor; hotkey `/` foca.
- **Som do +1** (`src/lib/sound.ts`): toca `public/plus-one.mp3` no clique do +1. O arquivo não está no repo — o time coloca o MP3 que quiser; ausente = silêncio sem erro (play rejeitado é engolido). Volume 0.6; cliques rápidos reiniciam o som de propósito.
- **Form em modal**: `<dialog>` nativo (`PhraseFormModal`) com o `PhraseForm` montado só com o modal aberto (senão o Mod+Enter dele dispararia fechado); `Mod+K` abre; prop `onSuccess` fecha após persistir. Estilos `.app-modal`/`::backdrop`.
